...
func Open(path string) (*Store, error) {
	...
	for _, p := range []string{
		...
		// Сводки считают count(DISTINCT) и оконные функции: под них SQLite
		// строит временные B-деревья и по умолчанию уносит их файлами в
		// /var/tmp. На проекте с тремя миллионами событий в сутки это давало
		// пятнадцать мегабайт записи в секунду при пересчёте раз в минуту.
		"PRAGMA temp_store = MEMORY",
		// Кэш страниц по умолчанию — два мегабайта, и база на несколько
		// гигабайт перечитывает один и тот же индекс с диска каждый раз.
		// Значение со знаком минус SQLite читает как килобайты, и память
		// занимается по мере надобности, а не сразу при запуске.
