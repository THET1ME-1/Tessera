...
// Фильтр отвечает, показывать ли модуль приложению. Ядро про модули не знает
// ничего, кроме этого вопроса, — иначе пакет блоков потянул бы за собой
// загрузку манифестов.
type Фильтр func(app, модуль string) bool

// Для — фильтр, привязанный к одному приложению.
func (ф Фильтр) Для(app string) func(string) bool {
	if ф == nil {
		return nil
	}
	return func(модуль string) bool { return ф(app, модуль) }
}

// ИзМодуля — величина, которую модуль считает лучше ядра.
//
// [свой] отвечает, показывать ли этот модуль ЭТОМУ приложению: число учёток
// Togetherly в панели Wallet — не «неточность», а чужая цифра. Так и вышло
// 18.09.2026: у Wallet не было ни одного человека, а крупная цифра обзора
// показывала 150 649.
func ИзМодуля(s *store.Store, величина string, свой func(модуль string) bool) (float64, bool) {
	var адрес string
...
func Герой(s *store.Store, свой Фильтр) map[string]Source {
	return map[string]Source{
		...
		"people_hero": func(app, from, to string) (any, error) {
			...
			if всего, есть := ИзМодуля(s, "people_total", свой.Для(app)); есть {
				out.Value = всего
