...
// пустойСтенд — приложение есть, событий нет. Для проверок, где важно
// отсутствие данных, а не их содержимое.
func пустойСтенд(t *testing.T) *store.Store {
	t.Helper()
	s, err := store.Open(filepath.Join(t.TempDir(), "t.db"))
	if err != nil {
		t.Fatal(err)
...
func TestПриростНовыхПустБезПрошлогоОтрезка(t *testing.T) {
	// Стенд намеренно ПУСТОЙ. Общий стенд держит события за фиксированный
	// день, и пока он совпадал с «сегодня», прошлый отрезок был пуст. Стоило
