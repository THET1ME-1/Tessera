...
def лента(параметры):
    ...
    прогреть([ид for ид, *_ in строки], ист, ждать=True)
...
def прогреть(ids, ист, ждать=False):
    ...
    Ждём только первые шестнадцать кадров, остальное уходит генератору фоном.
    Целая холодная страница (лента 18+ теперь уводит в записи годовой
    давности, где кэша нет вовсе) в отведённое время не укладывается, а ядро
    ждёт модуль пятнадцать секунд и обрывает запрос — лучше отдать верх
    страницы сразу. Низ дорисуется поштучно, при запросе картинки.
    ...
    if not нет:
        return
    сразу, потом = (нет[:16], нет[16:60]) if ждать else ([], нет[:60])
    try:
        if сразу:
...
def лента_с_замком(страница, вид, ист, db, пара=""):
    ...
    прогреть([ид for _, ид, *_ in окно], ист, ждать=True)
    ...
    for группа, один, много, автор, когда in строки:
        for ссылка in кадры_записи(один, много):
            ...
            элементы.append({
                ...
                "file": файл,
                "at": (str(когда) or "")[:10],
                "video": файл.lower().endswith((".mp4", ".mov", ".webm")),
            })
