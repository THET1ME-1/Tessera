...
def прогреть(ids, ист, ждать=False):
    ...
    if not нет:
        return
    аргументы = [*команда, "--ids", ",".join(нет[:60])]
    try:
        if ждать:
            subprocess.run(аргументы, timeout=12, check=False,
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
            subprocess.Popen(аргументы, stdout=subprocess.DEVNULL,
...
def лента_с_замком(страница, вид, ист, db):
    ...
    прогреть([ид for ид, *_ in окно], ист, ждать=True)
